...
In this post we have gone over two popular generative model. One is the VAE and the other is the GAN. The VAE uses something called density approximation while the GAN use a direct approach that was rooted in game theory. The VAE usually generate a more blurry picture than the GAN. However it does have more control than the plain GAN since one can manipulate the latent vector. The GAN on the other hand generate more realistic images but loses the control to latent vectors.
